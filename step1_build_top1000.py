...
def main():
    ...
    if os.path.exists(CACHE_FILE):
        try:
            done = set(pd.read_csv(CACHE_FILE).ticker)
        except:
            done = set()
    ...
    for t in tqdm(todo):
        sym = f"{t}.US"
        try:
            klines = ctx.candlesticks(sym, "day", "2021-01-01", "2025-12-31", adjust="forward")
            year_sum = {}
            for k in klines:
                y = k.timestamp.year
                if y in YEARS:
                    year_sum[y] = year_sum.get(y, 0) + float(k.turnover)

            rows = [{"year":y, "symbol":sym, "turnover":v} for y,v in year_sum.items()]
            pd.DataFrame(rows).to_csv(CACHE_FILE, mode="a", header=not os.path.exists(CACHE_FILE), index=False)
        except:
            pass
        time.sleep(DELAY)
    ...
    final = []
    for y in YEARS:
        top = df[df.year==y].sort_values("turnover", ascending=False).head(TOP_N)
        final.append(top)
